  ...
  const toolResults = await Promise.all(
    toolCalls.map(async (call) => {
      ...
      try {
        ...
        if (call.name === "call_drone_api") {
          const response = (result as any)?.body ?? result;
          const flag =
            (result as any)?.flag && typeof (result as any).flag === "string"
              ? (result as any).flag
              : undefined;
          submitResult = { response, flag };
          if (flag) {
            console.log("🎉🚩 drone flag detected:", flag);
          }
        }
