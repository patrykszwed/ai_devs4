...
async function main() {
  ...
  if (!apiKey) {
    console.warn(
      "AI_DEVS_API_KEY nie ustawiony – pomijam wysyłkę do huba kursu",
    );
    return;
  }
  const verifyRes = await fetch(hubVerifyUrl(), {
    method: "POST",
  ...
  console.log("Wysłano odpowiedź do weryfikacji na hubie");
