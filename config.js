...
export const EXTRA_API_HEADERS =
  AI_PROVIDER === "openrouter" ? OPENROUTER_EXTRA_HEADERS : {};
...
const normalizeWebSearchConfig = (webSearch) => {
  ...
  if (webSearch.searchContextSize !== undefined) {
    ...
    if (!VALID_OPENAI_SEARCH_CONTEXT_SIZES.has(searchContextSize)) {
      throw new Error(
        'webSearch.searchContextSize must be one of: "low", "medium", "high"',
      );
    }
  ...
  if (AI_PROVIDER === "openrouter") {
    ...
    request.plugins = mergeOpenRouterPlugins(request.plugins, {
      ...
      ...(webSearchConfig.maxResults
        ...
        : {}),
      ...(webSearchConfig.searchPrompt
        ? { search_prompt: webSearchConfig.searchPrompt }
        : {}),
    });
